...
export default async function makeConfig(config, options) {
  ...
  const libbyConfig = {
    mode,
    entry: {
      main: [...devServerEntries, resolve(config.root, 'src/ui/index.js')],
      preview: [
        ...devServerEntries,
        resolve(config.root, 'src/preview/index.js')
      ]
    },
    output: {
    ...
    resolve: {
      ...
      alias: {
        ...
        // __LIBBY_CONFIG__: resolve(config.root, 'config.runtime.js'),
        __LIBBY_LAYOUT__: config.layout
          ? resolve(config.cwd, config.layout)
          : resolve(config.root, 'src/preview/defaultLayout.js'),
        __LIBBY_PREVIEW__: config.preview
          ? resolve(config.cwd, config.preview)
          : resolve(config.root, 'src/preview/defaultPreview.js'),
    ...
    plugins: [
      new HtmlWebpackPlugin({
        ...
        chunks: ['main'],
        filename: 'index.html',
        favicon: resolve(config.root, 'src/favicon.png')
      }),
      new HtmlWebpackPlugin({
      ...
      //   __LIBBY_ENTRIES__: JSON.stringify(config.entries)
      // }),
